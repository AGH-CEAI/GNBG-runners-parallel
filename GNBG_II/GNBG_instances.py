...
class GNBG:
    def __init__(self, MaxEvals, AcceptanceThreshold, Dimension, CompNum, MinCoordinate, MaxCoordinate, CompMinPos, CompSigma, CompH, Mu, Omega, Lambda, RotationMatrix, OptimumValue, OptimumPosition):
        ...
        self.FEhistory = np.empty(1000000)  # Pre-allocate
        self.FE = 0
    ...
    def fitness(self, X):
        ...
        for jj in range(SolutionNumber):
            ...
            for k in range(self.CompNum):
                ...
                f[k] = (self.CompSigma[k] + (a @ np.diag(self.CompH[k, :]) @ b).item() ** self.Lambda[k]).item()

            result[jj] = np.min(f)
            if self.FE >= len(self.FEhistory):
                return result
...
class GNBG_Recorder:
    def __init__(self, MaxEvals, AcceptanceThreshold, Dimension, CompNum, MinCoordinate, MaxCoordinate, CompMinPos, CompSigma, CompH, Mu, Omega, Lambda, RotationMatrix, OptimumValue, OptimumPosition):
        ...
        self.OptimumPosition = OptimumPosition
        self.FEhistory = np.empty(1000000)  # Pre-allocate
        self.FEsamples = np.empty((1000000,Dimension))
        self.FE = 0
        self.AcceptanceReachPoint = np.inf
        self.BestFoundResult = np.inf
    ...
    def fitness(self, X):
        if len(X.shape)<2:
            X = X.reshape(1,-1)
        SolutionNumber = X.shape[0]
        self.FEsamples[self.FE:self.FE+SolutionNumber,:]=X
        result = np.nan * np.ones(SolutionNumber)
        for jj in range(SolutionNumber):
            x = X[jj, :].reshape(-1, 1)  # Ensure column vector
            f = np.nan * np.ones(self.CompNum)
...
def get_gnbg(ProblemIndex,save=False):
    ...
    if save:
        return GNBG_Recorder(MaxEvals, AcceptanceThreshold, Dimension, CompNum, MinCoordinate, MaxCoordinate, CompMinPos, CompSigma, CompH, Mu, Omega, Lambda, RotationMatrix, OptimumValue, OptimumPosition)
...
if __name__ == "__main__":
    ...
    # best_start_value=min(gnbg.FEhistory[:250])
    # start_difference=best_start_value-gnbg.OptimumValue
    # best_value=min(gnbg.FEhistory[:gnbg.MaxEvals])
