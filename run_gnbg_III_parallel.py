...
from GNBG_III.gnbg_iii_competition_harness import load_gnbg_problem
from GNBG_III.fitness import fitness
...
def _run_one(fid, budget):
    ...
    problem = load_gnbg_problem(fid)
    algorithm = _worker_metaheuristic(dim=problem["Dimension"])
    stopping_condition = lambda: problem.FE >= budget
    fitness_for_algorithm = lambda x:fitness(x,problem)
    algorithm(fitness_for_algorithm, stopping_condition)
